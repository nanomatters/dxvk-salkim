...
- `gpuname`: Displays the name of the GPU in use.
- `gputemp`: Displays the temperature of the GPU in use.
- `gpupower`: Displays the power draw of the GPU in use, in watts.
- `systeminfo`: Displays the CPU, Proton and Wine versions, and window system.
...
- `center`: Centers the horizontal row.

Additionally, `DXVK_HUD=1` has the same effect as `DXVK_HUD=devinfo,fps`, and `DXVK_HUD=full` enables all available HUD elements.
For example, `DXVK_HUD=systeminfo,fps,gpuload,horizontal,center` displays those values in a centered row.
